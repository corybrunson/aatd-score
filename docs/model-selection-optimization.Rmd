...
read_rds(here::here("data/aatd-2-fr-eval-old.rds")) %>%
  mutate(across(c(model, predictors, response), fct_inorder)) %>%
  group_by_at(vars(-id, -fold, -number)) %>%
...
Logistic regression models clearly achieve greater performance in this setting than random forest models, which are only marginally better than chance.
