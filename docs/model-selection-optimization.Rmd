...
With two exceptions, they are comparable in performance within each predictor set.
We can identify the two lower-performing specifications in the area-under-the-curve table below.
...
While the best-performing logistic regression models on average outperform the FasterRisk models, the logistic regression family also produces the worst-performing models when its regularization penalty is high and its point bound is small.
...
Because `FasterRisk` returns 2 models per specification, we represent each representation using "dodging" histograms in the same plotting window.
...
* The logistic regression model with low penalty and coefficient bound 25, which has an AUROC of 0.642 using 12 items.
* One FasterRisk model with 10 items and a coefficient bound of 100, which has an AUROC of 0.640 and two fewer items than the aforementioned logistic regression model that matches its performance; the omitted items are family history of AAT deficiency and hepatitis.
* One FasterRisk model with 10 items and a coefficient bound of 25, with an AUROC of 0.629 and in fact a largest coefficient of only 14; compared to the aforementioned FasterRisk model, it exchanges jaundice for hepatitis.
* One logistic regression model and several equivalent FasterRisk models with AUROC of 0.613 and only 6 items, all included in each of the aforementioned models: AAT deficiency (self), liver transplantation, cirrhosis, bronchiectasis, emphysema, and abnormal LFT.

The relative magnitudes of the point values are visually compared in the plot below.

```{r coefficient plot, fig.height=7}
eval_points %>%
