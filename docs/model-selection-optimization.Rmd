...
for (r in seq(nrow(eval_cands))) {
  eval_cands %>%
    ...
    unnest(cutoff) %>%
    mutate(counts = map(predictions, ~ count(., score))) %>%
    mutate(counts = map(counts, ~ arrange(., desc(score)))) %>%
    mutate(counts = map(counts, ~ mutate(., screen = cumsum(n) / sum(n)))) %>%
    mutate(counts = map(counts, ~ arrange(., score))) %>%
    mutate(counts = map2(score, counts, ~ filter(.y, score == .x))) %>%
    mutate(counts = map(counts, ~ select(., screen))) %>%
    unnest(counts) %>%
    transmute(
      Score = str_c(
      ...
      Risk = str_c(
        format(round(risk, digits = 3L) * 100, nsmall = 1L, trim = TRUE), "%"
      ),
      Screen = str_c(
        format(round(screen, digits = 3L) * 100, nsmall = 1L), "%"
      )
    ) %>%
    knitr::kable(
      align = "lrr",
...
```{r plot separate ROC curves with score labels, fig.height=8}
eval_cands %>%
  mutate(predictions = map(
    predictions,
    ~ rename_with(., function(x) str_remove(x, "^geno_"))
  ...
  coord_equal() +
  geom_path() +
  geom_text(aes(label = label), hjust = "left", vjust = "bottom") +
  geom_abline(intercept = 1, slope = -1, lty = 3) +
  labs(color = NULL)
```
...
eval_cands %>%
  ...
  transmute(
    Candidate = candidate,
    `Score cutoff` = as.integer(cutoff), `Risk cutoff` = .threshold,
...
eval_cands %>%
  ...
  ungroup() %>%
  pivot_wider(c(Candidate, Screen),
