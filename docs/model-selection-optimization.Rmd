...
While FR does not measure up when predicting ZZ, especially from only medical history, it becomes more competitive as all models weaken on the other predictive tasks.
...
The greatest improvements in ZZ prediction over random screening are limited to the LR, LDA, NN, SVM, and FR models and range from 15 to 25 percentage points.
These obtain near 50% specificity, which makes for useful comparisons against COPD-based screening.
For ZZ prediction (see the plot), the LR, LDA, and FR models obtain between 9 and 21 percentage point improvement over COPD-based screening in sensitivity subject to a specificity loss of less than 5 percentage points.
...
We see AUROC improvements of between $0.03$ and $0.04$ from the strictest to the loosest parameter settings.
...
```{r compare coefficients of candidate models}
eval_data %>%
...
eval_points %>%
  ...
  pivot_wider(c(model, specification, number, AUROC),
...
* The logistic regression model with low penalty and point value bound 25, which has an AUROC estimate of 0.576 using 9 (of 10) items. It matches the performance of two more complicated models, one with higher point values and the other with an additional term (family history of COPD).
* The FasterRisk model with 7 items and a point value bound of 100 (though actual highest point value 13), with estimated AUROC 0.575. It omits hepatitis and COPD as well as family history of COPD and matches the performance of some more complicated models.
* An assortment of FasterRisk models with 6 terms each and point values below 10 that achieve AUROC estimates of 0.565. Compared to the above, these additionally exclude allergies as an item, as well as hepatitis, COPD, and family history of COPD. This may be valuable in that, when used, allergies and asthma have negative point values.
* The FasterRisk model with 10-term and 6-point value limits that achieves the AUROC estimate 0.550. It uses only 6 terms, excluding allergies and asthma but including hepatitis, with positive point values at most 5, so is the simplest of the candidates (excluding the poorly-performing LR model).

### ZZ cases recovered by inclusion of smoking history
