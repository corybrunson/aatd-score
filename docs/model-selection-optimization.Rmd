...
Models were evaluated using a fixed sample of $\frac{1}{`r sample_denom`}$ of the data.
...
```{r roc plots for ML, fig.height=8}
aatd_mod_res_pred %>%
...
```{r roc plots for FR, fig.height=8}
aatd_fr_res_pred %>%
  group_by(model, predictors, response) %>%
  roc_curve(truth = geno_class, estimate = .pred_Abnormal) %>%
  ungroup() %>%
...
```{r pr plots for ML, fig.height=6}
aatd_mod_res_pred %>%
  left_join(model_types, by = "model") %>%
  filter(type != "rule/tree-based") %>%
  group_by(model, predictors, response) %>%
...
```{r pr plots for FR, fig.height=6}
aatd_fr_res_pred %>%
  group_by(model, predictors, response) %>%
  pr_curve(truth = geno_class, estimate = .pred_Abnormal) %>%
  ggplot(aes(x = recall, y = precision)) +
...
First, the FasterRisk program returns a collection of similarly-performing models rather than a single best-fit model. This is in part because the underlying space of possible models is not approximately continuous but coarsely discrete: Whereas the coefficients of LR and the tree weights of RF can be continuously tuned, the integer coefficients of FR models cannot. Moreover, two similarly-performing risk scores may assign meaningfully different weights to the same or even different sets of predictors, so the selection of a "best" model from among this collection has less to do with a marginal improvement in some performance measure than with the usefulness of its interpretation.

## Comparison of model families

Across the same sets of predictors and responses as in Phase 1, we evaluated LR and RF models using 6-fold cross validation on a $\frac{4}{5}$ training set.
...
Most FR models improved still upon the RF models, but with much more variability in performance.
On the more important measure of AUROC, however, LR models universally outperformed RF models, and even outperformed most FR models.
...
aatd_ml_metrics %>%
  ...
  mutate(param_pref = case_when(
    ! is.na(penalty) ~ rank(penalty),
    ! is.na(trees) ~ rank(desc(trees)),
    ! is.na(terms) ~ rank(terms),
    ! is.na(bound) ~ rank(bound)
  )) %>%
...
It appears that the number of terms is more consequential than the point bounds, though we compared models using a smaller spread of the latter.
